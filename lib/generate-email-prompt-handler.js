...
import { chargeRequestAfterSuccess, createAdminClient } from "./points.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const {
      level = "Medium",
      topic = "Mixed",
      personalizedRecommendationId = null,
    } = req.body || {};

    if (personalizedRecommendationId) {
      await requireActivePro(createAdminClient(), req);
    }
...
`;
    ...
    const creditBalance = await chargeRequestAfterSuccess(
      req,
      EMAIL_PROMPT_COST,
    ...
    return res.status(200).json({
      ...
      category: topic !== "Mixed" ? topic : json.category || null,
      knowledgeCategory: json.knowledgeCategory || "内容完整性",
