alter table public.redeem_codes
  ...
  add column if not exists allowed_user_id uuid references auth.users(id) on delete cascade;

alter table public.redeem_codes
  drop constraint if exists redeem_codes_repeatable_requires_user;

alter table public.redeem_codes
  add constraint redeem_codes_repeatable_requires_user
    check (not is_repeatable or allowed_user_id is not null);

alter table public.redeem_logs
  drop constraint if exists redeem_logs_user_id_code_id_key;

create index if not exists redeem_logs_user_code_lookup_idx
  on public.redeem_logs (user_id, code_id);
...
begin
  ...
  if code_row.allowed_user_id is not null
     and code_row.allowed_user_id <> target_user_id then
    raise exception using errcode = 'P0001', message = 'INVALID_CODE';
  end if;
  ...
  if not code_row.is_repeatable and code_row.used_count >= code_row.max_uses then
    raise exception using errcode = 'P0001', message = 'CODE_LIMIT_REACHED';
  end if;

  if not code_row.is_repeatable and exists (
    select 1
  ...
  set used_count = used_count + 1,
      is_active = case
        when is_repeatable then is_active
